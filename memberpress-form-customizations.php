...
<script>
 	jQuery(document).ready(function(){
 		jQuery('.mepr-payment-methods-wrapper').append(jQuery('input[type="checkbox"]').parent().parent());
 	})
</script>
<!-- end rendering checkboxes at the bottom of the form" -->

<div class="custom-form-msg">
	<H3 class="custom-form-title">Organizations' Authorized User</H3>
	<span>Subscription to this platform requires authorized user registration below:</span>
</div>
...
<div class="mp_wrapper">
  <form class="mepr-signup-form mepr-form" method="post" action="<?php echo $_SERVER['REQUEST_URI'].'#mepr_jump'; ?>" enctype="multipart/form-data" novalidate>
    ...
    <?php endif; ?>
	  ...
	  <!-- new location of custom fiields -->
  <?php MeprHooks::do_action('mepr-checkout-before-custom-fields', $product->ID); ?>
    ...
    <?php MeprHooks::do_action('mepr-checkout-after-custom-fields', $product->ID); ?>
      <!-- end of new location of custom fields -->
      ...
      <div class="mp-form-row mepr_tos">
        <label for="mepr_agree_to_tos<?php echo $unique_suffix; ?>" class="mepr-checkbox-field mepr-form-input" required>
          <input type="checkbox" name="mepr_agree_to_tos" id="mepr_agree_to_tos<?php echo $unique_suffix; ?>" <?php checked(isset($mepr_agree_to_tos)); ?> />
			<?php echo stripslashes($mepr_options->tos_title); ?>
			<a href="https://steamondemand.org/terms-of-service-211006/" target="_blank" rel="noopener noreferrer"> STEAM On Demand℠ Service Agreement.</a>
        </label>
      </div>
    <?php endif; ?>
    ...
    <?php if($mepr_options->require_privacy_policy && $privacy_page_link = MeprAppHelper::privacy_policy_page_link()): ?>
      <div class="mp-form-row">
        <label for="mepr_agree_to_privacy_policy<?php echo $unique_suffix; ?>" class="mepr-checkbox-field mepr-form-input" required>
          ...
          <?php echo preg_replace('/%(.*)%/', '<a href="' . $privacy_page_link . '" target="_blank">$1</a>', __($mepr_options->privacy_policy_title, 'memberpress')); ?> and the
          <a href="<?php echo stripslashes($mepr_options->tos_url); ?>" target="_blank" rel="noopener noreferrer">End User Agreement.</a>
        </label>
      </div>
    <?php endif; ?>
